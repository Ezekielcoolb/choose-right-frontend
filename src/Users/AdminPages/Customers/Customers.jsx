...
import { fetchCustomers, deleteCustomer, deleteCustomers } from "../../../redux/slices/customersSlice";
...
export default function AdminCustomers() {
  ...
  const { items, status, error, pagination, totals, mutationStatus, mutationError } = useSelector((state) => state.customers);
  ...
  useEffect(() => {
    dispatch(
      fetchCustomers({
        ...
        csoId: csoFilter === "all" ? undefined : csoFilter,
        page,
        limit: pageSize,
      }),
    );
  }, [debouncedSearch, csoFilter, page, dispatch]);

  useEffect(() => {
    setSelectedIds([]); // Clear selection when filters or page change
  }, [debouncedSearch, csoFilter, page]);
  ...
  const aggregated = useMemo(() => {
    return totals || { totalDeposited: 0, availableBalance: 0, totalFees: 0, totalWithdrawn: 0, activePlans: 0 };
  }, [totals]);
  ...
  const confirmDeleteCustomer = async () => {
    if (!customerToDelete && selectedIds.length === 0) return;

    try {
      if (customerToDelete) {
        await dispatch(deleteCustomer({ customerId: customerToDelete._id, admin: true })).unwrap();
      } else {
        await dispatch(deleteCustomers({ customerIds: selectedIds, admin: true })).unwrap();
        setSelectedIds([]);
      }
      closeDeleteModal();
    } catch (deleteError) {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === paginatedCustomers.length) {
      ...
      setSelectedIds(paginatedCustomers.map((c) => c._id));
    }
  };
  ...
  return (
    <div className="space-y-6 p-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {/* {selectedIds.length > 0 && (
              <button
                type="button"
                onClick={() => {
                  setCustomerToDelete(null); // Signal bulk delete
                  setShowDeleteModal(true);
                }}
                className="flex items-center gap-2 rounded-full bg-rose-50 px-4 py-2 text-sm font-semibold text-rose-600 transition hover:bg-rose-100"
              >
            ...
            <div className="space-y-4">
              ...
              {pagination?.total > pageSize ? (
                <div className="flex flex-col gap-3 border-t border-slate-100 pt-4 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between">
                  <p>
                    Showing {paginatedCustomers.length ? (page - 1) * pageSize + 1 : 0}-
                    {Math.min(page * pageSize, pagination?.total)} of {pagination?.total} customers
                  </p>
      ...
      {showDeleteModal && (customerToDelete || selectedIds.length > 0) ? (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2">
                <h2 className="text-lg font-semibold text-slate-900">
                  {customerToDelete ? "Delete customer?" : `Delete ${selectedIds.length} customers?`}
                </h2>
                <p className="text-sm text-slate-600">
                  {customerToDelete
                    ...
                    : `This will permanently remove the ${selectedIds.length} selected customers and all their associated savings plans and records. This action cannot be undone.`}
                </p>
                {mutationError ? (
